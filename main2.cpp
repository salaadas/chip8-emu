...
constexpr int ADDRESS_MASK = 0xFF; // prevent overflowing
...
void emulate(void) // emulate one cycle
{
    ...
    switch (opcode & 0xF000) {
        ...
        case 0x7000: {
            printf("-- 0x7XNN add NN to Vx (carry flag unchanged) ---\n");
        ...
        case 0x8000: {
            switch(opcode & 0x000F) {
                ...
                case 0x0001: {
                    printf("--- 0x8XY1 set Vx |= Vy (bitwise OR) ---\n");
                    V[(opcode & 0x0F00) >> 8] |= V[(opcode & 0x00F0) >> 4];
                    pc += 2;
                } break;
                // 0x8XY2 set Vx &= Vy (bitwise AND)
                case 0x0002: {
                    printf("--- 0x8XY2 set Vx &= Vy (bitwise AND) ---\n");
                    V[(opcode & 0x0F00) >> 8] &= V[(opcode & 0x00F0) >> 4];
                    pc += 2;
                } break;
                // 0x8XY3 set Vx ^= Vy (bitwise XOR)
                case 0x0003: {
                    printf("--- 0x8XY3 set Vx ^= Vy (bitwise XOR) ---\n");
                    V[(opcode & 0x0F00) >> 8] ^= V[(opcode & 0x00F0) >> 4];
                    pc += 2;
                } break;
                // 0x8XY4 set Vx += Vy
        ...
        case 0xF000: {
            switch (opcode & 0x00FF) {
                ...
                case 0x0029: {
                    I = (V[(opcode & 0x0F00) >> 8] & 0xF) * 5; // 0x50 +
                    pc += 2;
                } break;
                case 0x0033: {
                    int number = V[(opcode & 0x0F00) >> 8];
                    for (int i = 2; i >= 0; --i) {
                        memory[(I + i) & ADDRESS_MASK] = (int)(number % 10);
                        number /= 10;
                    }
                    pc += 2;
                } break;
                case 0x0055: {
                    for (int offset = 0; offset <= V[(opcode & 0x0F00) >> 8]; ++offset) {
                        memory[(I + offset) & ADDRESS_MASK] = V[offset];
                    }
                    pc += 2;
                } break;
                case 0x0065: {
